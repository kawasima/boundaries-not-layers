...
/**
 * 性能+完全性版のエンドツーエンドテスト（実 H2 + jOOQ）。
 * 元 TS の victim_purity.test.ts に相当。上限は 10000。
 */
@SpringBootTest
...
class CartIntegrationTest {
    ...
    @Test
    void 追加した中身をページ参照できる() throws Exception {
        mockMvc.perform(post("/carts/items").contentType(MediaType.APPLICATION_JSON)
                        .content(body(USER, ON_SALE, 3)))
                .andExpect(status().isCreated());

        mockMvc.perform(get("/carts/items").param("userId", USER).param("page", "0").param("size", "20"))
